...
function Viper(options, callback)
{
    ...
    var self     = this;
    this.load(viperURL, function() {
        if (options) {
            self._processOptions(options, function() {
...
Viper.prototype = {
    ...
    /**
     * Loads required Viper script files.
     *
     * After creating a new Viper object, this method must be called to load
     * scripts that Viper depends on.
     *
     * @param {string}   url      The base URL for viper.
    ...
    {
        var self = this;
        this.loadScripts(url, dfx.clone(this.scripts), function() {
            self.setViperURL(url);
            self.setStylesURL(url);
            self.init();
            callback.call(self);
        });
     ...
     * @return {void}
     */
    loadScripts: function(url, scripts, callback)
    {
        if (scripts.length === 0) {
            callback.call(this);
            return;
        ...
        if (window[script]) {
            this.loadScripts(url, scripts, callback);
        } else {
            var self = this;
            dfx.includeScript(url + '/' + script + '.js', function() {
                self.loadScripts(url, scripts, callback);
            });
        }
    ...
    setPluginSets: function(sets, callback)
    {
        ...
        for (var setName in sets) {
            ...
            ViperPluginManager.addPluginSet(setName, set.plugins, true, function() {
                delete sets[setName];
    ...
    init: function()
    {
        ViperUndoManager.init(this);
        ViperPluginManager.init(this);
    ...
    _addEvents: function(elem)
    {
        ...
        // Drag & Drop
        // TODO: we are going to disable editing when a drag event starts.
        // For some reason there is no way to get a reference to dropped element
        // (at least not in Firefox that i can see) in contentEditable element.
        // If an image is dragged, the dropped event should contain a reference to the
        // element.. Also trying to dataTransfer.clearData() fails for this document,
        // I am guessing its also cause of contentEditable=true..
        this._document.ondragstart = function(e) {
            elem.setAttribute('contentEditable', false);
        }

        this._document.ondragend = function(e) {
    ...
    setEditableElement: function(elem)
    {
        ...
        ViperUndoManager.setActiveElement(elem);
        this.inlineMode = false;
        elem.setAttribute('contentEditable', true);
    ...
    insertNodeAtCaret: function(node)
    {
        ...
        if (typeof node === 'string') {
            ...
            if (ViperPluginManager.fireNodeInserted(newNode, newRange) === false) {
                noBlock = false;
            }
            ...
            this.fireNodesChanged('Viper:insertNode', false, true);
    ...
    /**
     * Converts a specified color to the inverse of that color.
     *
     * @param string color A hexidecimal representation of the color to convert.
     *
     * @return string A hexidecimal representation of the inverse color.
     */
    getInverseColor: function(color)
    {
        color = color.toLowerCase();

        var forward  = '0123456789abcdef#';
        var backward = 'fedcba9876543210#';

        var color = color.replace(/./gi, function (chr) {
            return backward.charAt(forward.indexOf(chr));
        });
    ...
    _getInsertedInfo: function(node)
    {
        return {
         type: 2,
         node: node,
    ...
    _getRangeInfo: function(range)
    {
        ...
        var info   = {
            ...
            before: dfx.getHtml(commEl),
            path: XPath.getPath(commEl),
            after: '',
    ...
    deleteContents: function(right)
    {
        ...
        if (range.collapsed === false) {
            ...
            if (container.nodeType === dfx.ELEMENT_NODE) {
                ...
                if (right === true) {
                    // Remove content from the right of the caret (backspace).
    ...
    getNextSiblingElementNode: function (node)
    {
        while (node.nextSibling) {
            if (node.nextSibling.nodeType === dfx.ELEMENT_NODE) {
    ...
    _wrapElement: function(parent, tag, callback)
    {
        ...
        if (parent.nodeType === dfx.TEXT_NODE) {
            if (dfx.isBlank(parent.textContent) !== true) {
    ...
    /**
     * Returns the most outer inline tag.
     * E.g. <span>text<strong><i><span>text2</span></i></strong></span> given text2
     * then will return <strong>. This is for removeStyles().
     */
    _getOuterTag: function(node, right)
    {
        if (right !== true) {
            if (!node.previousSibling
                || (node.previousSibling.nodeType === dfx.TEXT_NODE
                    && dfx.isBlank(node.previousSibling.data) === true)
            ) {
                var parent = node.parentNode;
                if (parent && dfx.isBlockElement(parent) === false) {
                    return this._getOuterTag(parent, right);
                }
            }
        } else {
            if (!node.nextSibling
                || (node.nextSibling.nodeType === dfx.TEXT_NODE
                    && dfx.isBlank(node.nextSibling.data) === true)
            ) {
                var parent = node.parentNode;
                if (parent && dfx.isBlockElement(parent) === false) {
                    return this._getOuterTag(parent, right);
    ...
    */
    ...
    _handleAncillaryKey: function(e)
    {
        ...
        switch (key) {
            case dfx.DOM_VK_DELETE:
                this.deleteContents();
                ViperPluginManager.fireKeyPressed(e);
            break;
            ...
            case dfx.DOM_VK_RIGHT:
                ViperPluginManager.fireCaretPositioned();
    ...
    keyDown: function(e)
    {
        if (this.pluginActive() === true && ViperPluginManager.allowTextInput !== true) {
            return;
        }

        if (!ViperPluginManager.fireKeyDown(e)) {
            return false;
        }
        ...
        if (this._handleSpecialKey(e) === false) {
            ...
            if (!ViperPluginManager.fireKeyPressed(e)) {
                return false;
            }
        }
    ...
    dragStart: function(e)
    {
        ...
        this.fireCallbacks('dragStart', {e: e}, function() {
        });
    ...
    dragDrop: function(e)
    {
        ...
        this.fireCallbacks('dragDrop', {e: e}, function() {
        });
    },
    ...
    {
        if (ViperPluginManager.fireClicked(e) === false) {
            return false;
        }
        ...
        ViperPluginManager.fireSelectionChanged(range);
        this.fireCallbacks('caretPositioned', {range: range});

    },

    setStylesURL: function(url)
    {
        this.stylesURL = url;

        var viperCSS = this.stylesURL + '/viper.css';
        dfx._includeCss(viperCSS);

    },

    setViperURL: function(url, callback)
    {
        this.viperURL                 = url;
        ViperPluginManager.pluginsURL = url + '/plugins/';
    ...
    {
        ViperPluginManager.fireCaretUpdated();
    ...
    fireNodesChanged: function(type, noFocus, isTextChange)
    {
        ...
        ViperUndoManager.add('viper', type);
    ...
    {
        if (e) {
            if (ViperPluginManager.fireMouseDown(e) === false) {
                return false;
            }
        }
    ...
    {
        return (ViperPluginManager.getActivePlugin() !== null);

    },

    isPluginElement: function(element)
    {
        return ViperPluginManager.isPluginElement(element);
    ...
    _cleanDOM: function(elem)
    {
        ...
        if (elem.firstChild) {
            this._cleanDOM(elem.firstChild);
        }

        var nextSibling = elem.nextSibling;
        this._cleanNode(elem);

        if (nextSibling) {
            this._cleanDOM(nextSibling);
        }
    ...
    _cleanNode: function(node)
    {
        if (node.nodeType === dfx.ELEMENT_NODE) {
            ...
            switch (tagName) {
                ...
                default:
                    if (dfx.isStubElement(node) === false && !node.firstChild) {
                        if (tagName === 'span' && node.getAttribute('viperchangeid')) {
                            return;
                        }
            ...
            if (dfx.trim(node.data) === '' && dfx.isTag(node.parentNode, 'td') === false) {
                dfx.remove(node);
            }
        }
